...
class CalculateListSizeUseCase {
    ...
    operator fun invoke(list: List<List<PasswordUIModel>>): Int {
        ...
        if (list.size == 1) {
            ...
            return (firstList.size * PASSWORD_IDLE_ITEM_HEIGHT) + (expandedItem?.let { PASSWORD_EXPANDED_ITEM_HEIGHT + (additionalFieldsCount * ADDITIONAL_FIELD_HEIGHT) }
                ?: 0)
        }
        val firstList = list.first()
